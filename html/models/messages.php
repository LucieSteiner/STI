...
function get_message_detail($id){
    ...
    $result = $file_db->prepare('SELECT messages.id, title, time, message, users.login as sender FROM messages JOIN users ON users.id = messages.sender WHERE messages.id = ?');
